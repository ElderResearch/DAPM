...
#' @param db_schema  Where to write scores to. Default is nrd.
#' 
...
distance_score <- function(cycle_date = NULL, scoreTableName = "nrd.enty_score", inputDF=NULL) {
  
  # Basic APM init stuff
  if(APM::initialize_apm("Distance IP Scoring") != 0)
    return(1)
  ...
  if(is.null(inputDF)){
    ...
    ## Wait for Ben to populate ip_conn_type_cd: -- and ip_conn_type_cd != 'cel'
