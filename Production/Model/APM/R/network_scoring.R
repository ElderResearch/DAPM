...
#' @param ipMonths Number of months to seek back for IP connections (actually
#'   seek back 4 * [number of months] weeks)
#'
...
#' @export
network_score <- function(cycle_date     = NULL,
                          prevDays       = 90,
  ...
  # Parameter setting
  # Number of quarters of employment history
  emplrQuarters <- set_param_num(param = "EMPLR_QUARTERS",
                                 con   = db_conn$conn,
                                 input = emplrQuarters)

  if ("error" %in% class(emplrQuarters)) {
    return(21)
  }

  # Number of years of PII history
  piiYears <- set_param_num(param = "PII_YEARS",
                            con   = db_conn$conn,
                            input = piiYears)

  if ("error" %in% class(piiYears)) {
    return(22)
  }

  # Number of months of IP history
  ipMonths <- set_param_num(param = "AGGR_CLAIMT_SESSN_MON_CNT",
                            con   = db_conn$conn,
                            input = ipMonths)

  if ("error" %in% class(ipMonths)) {
    return(23)
  }
  ...
  IP_MIN_DT <- as.character(as.Date(cycle_date) - lubridate:::weeks(4*ipMonths))
