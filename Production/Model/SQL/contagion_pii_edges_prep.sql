...
FROM    (SELECT COUNT(1) FROM claimt_attr WHERE claimt_attr_type_cd = 'PHON') as ph,
	claimt_attr			  AS a
...
FROM    (SELECT COUNT(1) FROM claimt_attr WHERE claimt_attr_type_cd = 'EMAIL') as em,
	claimt_attr			  AS a
	LEFT JOIN aggr_shared_claimt_attr AS b ON 
		a.claimt_attr_type_cd = b.claimt_attr_type_cd AND
		a.claimt_attr_val = b.claimt_attr_val
...
FROM    (SELECT COUNT(1) FROM claimt_attr WHERE claimt_attr_type_cd = 'PASSWD') as pw,
	claimt_attr			  AS a
	LEFT JOIN aggr_shared_claimt_attr AS b ON 
		a.claimt_attr_type_cd = b.claimt_attr_type_cd AND
		a.claimt_attr_val = b.claimt_attr_val
...
JOIN 
	...
	(SELECT t1.claimt_id AS from_temp,
		t1.claimt_sessn_tmstmp AS from_ts,
		COUNT(DISTINCT t2.claimt_id) as tot_count
	FROM nrd.aggr_claimt_sessn t1
	JOIN nrd.aggr_claimt_sessn t2
	ON
	t1.claimt_sessn_tmstmp > t2.claimt_sessn_tmstmp
	...
	t1.claimt_id <> t2.claimt_id
	WHERE (t1.claimt_sessn_tmstmp - t2.claimt_sessn_tmstmp) < '7 00:00:00'
		AND t1.claimt_id IN (SELECT DISTINCT t1.from FROM tmp.pairwise_subset t1)
	GROUP BY from_temp,
		 from_ts) AS tot_counts
ON
